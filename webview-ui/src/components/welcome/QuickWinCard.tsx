...
const QuickWinCard: React.FC<QuickWinCardProps> = ({ task, onExecute }) => {
	return (
		<div
			className="flex items-center mb-2 py-0 px-5 space-x-3 rounded-full cursor-pointer group transition-colors duration-150 ease-in-out bg-white/2 border border-(--vscode-panel-border) hover:bg-[var(--color-claude-clay)]/10 hover:border-[var(--color-claude-clay)]/50"
			onClick={() => onExecute()}>
			<div className="shrink-0 flex items-center justify-center w-6 h-6 text-(--vscode-icon-foreground) group-hover:text-[var(--color-claude-orange)] transition-colors">
